...
contract FundSubscription is Script {
    ...
    function fundSubscription(address vrfCoordinator, uint256 subId, address link, uint256 deployerKey) public {
        ...
        if (block.chainid == 31337) {
            vm.startBroadcast(deployerKey);
            VRFCoordinatorV2Mock(vrfCoordinator).fundSubscription(uint64(subId), FUND_AMOUNT);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast(deployerKey);
...
contract AddConsumer is Script {
    function addConsumer(address raffle, address vrfCoordinator, uint256 subId, uint256 deployerKey) public {
        ...
        vm.startBroadcast(deployerKey);
        if (block.chainid == 31337) {
            VRFCoordinatorV2Mock(vrfCoordinator).addConsumer(uint64(subId), raffle);
        } else {
            IVRFCoordinatorV2Plus(vrfCoordinator).addConsumer(subId, raffle);
        }
        vm.stopBroadcast();
    }
